...
class JobCategory extends Model
{
    ...
    protected \$fillable = ['kode', 'nama', 'divisi', 'aktif'];
}
PHP;
file_put_contents('c:/Users/achai/cbm/app/Models/JobCategory.php', $modelContent);
...
class Categories extends Component
{
    ...
    public \$category_id, \$kode, \$nama, \$divisi, \$aktif = 1;
...
PHP;
file_put_contents('c:/Users/achai/cbm/app/Livewire/Master/Categories.php', $livewireContent);
...
<div>
    <div class="cbm-page-header">
        <div style="display: flex; align-items: center; gap: 1rem;">
            <div style="width: 3.5rem; height: 3.5rem; border-radius: 1rem; background: linear-gradient(135deg, #f59e0b, #ea580c); display: flex; align-items: center; justify-content: center; color: white; flex-shrink: 0; box-shadow: 0 8px 16px rgba(245,158,11,0.3);">
            ...
            <div>
                <h1 class="cbm-greeting" style="font-size: 1.5rem;">Kategori Pekerjaan</h1>
                <p class="cbm-greeting-sub">Kelola kategori pekerjaan dan temuan</p>
            </div>
        </div>
    </div>
    ...
    <div class="cbm-card">
        <div class="cbm-card-header" style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--cbm-card-border); padding-bottom: 1.25rem; margin-bottom: 0;">
            <div style="position: relative; width: 300px;">
                ...
                <input type="text" placeholder="Cari Kategori..." class="cbm-input" style="padding-left: 2.75rem; width: 100%; height: 2.75rem; border-radius: .875rem; background: var(--cbm-bg); border: 1px solid var(--cbm-card-border); color: var(--cbm-text);" wire:model.live.debounce.300ms="search">
            </div>
            <button type="button" wire:click.prevent="create" class="cbm-btn cbm-btn-primary" style="background: linear-gradient(135deg, #f59e0b, #ea580c); box-shadow: 0 4px 12px rgba(245,158,11,0.3); border: none; color: white; padding: 0 1.5rem; height: 2.75rem; border-radius: .875rem; font-weight: 700; display: flex; align-items: center; gap: .5rem; cursor: pointer; transition: transform 0.2s, box-shadow 0.2s;">
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: left;">
                <thead>
                    <tr style="background: var(--cbm-bg); border-bottom: 1px solid var(--cbm-card-border);">
                        <th style="padding: 1rem 1.5rem; font-size: .75rem; font-weight: 700; color: var(--cbm-text-muted); text-transform: uppercase; letter-spacing: .05em;">Kode Kategori</th>
                        <th style="padding: 1rem 1.5rem; font-size: .75rem; font-weight: 700; color: var(--cbm-text-muted); text-transform: uppercase; letter-spacing: .05em;">Nama Kategori</th>
                        <th style="padding: 1rem 1.5rem; font-size: .75rem; font-weight: 700; color: var(--cbm-text-muted); text-transform: uppercase; letter-spacing: .05em;">Divisi</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid var(--cbm-card-border); transition: background 0.2s;">
                        <td style="padding: 1rem 1.5rem; color: var(--cbm-text); font-weight: 600;">{{ $cat->kode }}</td>
                        <td style="padding: 1rem 1.5rem; color: var(--cbm-text-sub);">{{ $cat->nama }}</td>
                        <td style="padding: 1rem 1.5rem; color: var(--cbm-text-sub);">
                            <span style="background: rgba(59,130,246,0.1); color: #3b82f6; padding: 0.25rem 0.75rem; border-radius: 999px; font-size: 0.75rem; font-weight: 700;">{{ $cat->divisi }}</span>
                        </td>
    ...
    <div style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 50;">
        <div style="background: var(--cbm-card-bg); width: 100%; max-width: 500px; border-radius: 1.25rem; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); overflow: hidden;">
            <div style="padding: 1.5rem; border-bottom: 1px solid var(--cbm-card-border); display: flex; justify-content: space-between; align-items: center;">
                <h3 style="font-size: 1.125rem; font-weight: 700; color: var(--cbm-text);">{{ $isEditMode ? 'Edit Kategori' : 'Tambah Kategori Baru' }}</h3>
            ...
            <div style="padding: 1.5rem;">
                <div style="margin-bottom: 1rem;">
                    <label style="display: block; font-size: .875rem; font-weight: 600; color: var(--cbm-text); margin-bottom: .5rem;">Kode Kategori</label>
                    <input type="text" wire:model="kode" class="cbm-input" style="width: 100%; padding: .75rem 1rem; border-radius: .75rem; background: var(--cbm-bg); border: 1px solid var(--cbm-card-border); color: var(--cbm-text);" placeholder="Contoh: CAT-01">
                    @error('kode') <span style="color: #ef4444; font-size: .75rem; margin-top: .25rem; display: block;">{{ $message }}</span> @enderror
                </div>
                <div style="margin-bottom: 1rem;">
                    <label style="display: block; font-size: .875rem; font-weight: 600; color: var(--cbm-text); margin-bottom: .5rem;">Nama Kategori</label>
                    <input type="text" wire:model="nama" class="cbm-input" style="width: 100%; padding: .75rem 1rem; border-radius: .75rem; background: var(--cbm-bg); border: 1px solid var(--cbm-card-border); color: var(--cbm-text);" placeholder="Contoh: Cabin Appearance">
                ...
                <div style="margin-bottom: 1rem;">
                    ...
                    <select wire:model="divisi" class="cbm-input" style="width: 100%; padding: .75rem 1rem; border-radius: .75rem; background: var(--cbm-bg); border: 1px solid var(--cbm-card-border); color: var(--cbm-text);">
                    ...
                    @error('divisi') <span style="color: #ef4444; font-size: .75rem; margin-top: .25rem; display: block;">{{ $message }}</span> @enderror
                </div>
                <div style="margin-bottom: 1rem;">
                    <label style="display: block; font-size: .875rem; font-weight: 600; color: var(--cbm-text); margin-bottom: .5rem;">Status</label>
                    <select wire:model="aktif" class="cbm-input" style="width: 100%; padding: .75rem 1rem; border-radius: .75rem; background: var(--cbm-bg); border: 1px solid var(--cbm-card-border); color: var(--cbm-text);">
                        <option value="1">Aktif</option>
                        <option value="0">Tidak Aktif</option>
                    </select>
                    @error('aktif') <span style="color: #ef4444; font-size: .75rem; margin-top: .25rem; display: block;">{{ $message }}</span> @enderror
                </div>
            </div>
            <div style="padding: 1.25rem 1.5rem; border-top: 1px solid var(--cbm-card-border); background: var(--cbm-bg); display: flex; justify-content: flex-end; gap: .75rem;">
                ...
                <button type="button" wire:click.prevent="{{ $isEditMode ? 'update' : 'store' }}" style="padding: .75rem 1.5rem; border-radius: .75rem; background: linear-gradient(135deg, #f59e0b, #ea580c); border: none; color: white; font-weight: 600; cursor: pointer; box-shadow: 0 4px 12px rgba(245,158,11,0.3);">{{ $isEditMode ? 'Simpan Perubahan' : 'Tambah Data' }}</button>
            </div>
        </div>
    </div>
    @endif
...
BLADE;
file_put_contents('c:/Users/achai/cbm/resources/views/livewire/master/categories.blade.php', $bladeContent);

echo 'Job Categories CRUD generated successfully!';
